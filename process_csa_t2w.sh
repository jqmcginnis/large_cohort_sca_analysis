...
file_totalseg_all="${file_t2w}_step2_output"
file_totalseg_discs="${file_t2w}_step1_levels"
...
(
    ...
    if [[ "${SPINEPS_AVAILABLE}" == "true" ]]; then
        ...
        spineps sample -ignore_bids_filter -ignore_inference_compatibility -i "${file_t2w}.nii.gz" -model_semantic t2w -model_instance instance
        ...
        # SPINEPS outputs to derivatives_seg/ with _mod-T2w naming
        spineps_base="$(basename "${file_t2w}")"
        # SPINEPS outputs to derivatives_seg/ — entity order varies, so glob for it
        file_spineps_spine=$(ls "$(dirname "${file_t2w}")"/derivatives_seg/*_seg-spine*msk.nii.gz 2>/dev/null | head -1)
