...
if command -v spineps &>/dev/null; then
    spineps sample -i "${file_stir}.nii.gz" -model_semantic t2w -model_instance instance

    file_spineps_spine="${file_stir}_seg-spine"
...
SPINEPS_QC_ARGS=""
if [[ -f "${file_stir}_seg-spineps-cord.nii.gz" ]]; then
    SPINEPS_QC_ARGS="--spineps-cord ${file_stir}_seg-spineps-cord.nii.gz --spineps-canal ${file_stir}_seg-spineps-cord-canal-union.nii.gz"
fi
